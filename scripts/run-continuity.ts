...
try {
  const { values } = parseArgs({ options: { live: { type: 'boolean', default: false }, provider: { type: 'string', default: 'openrouter' }, split: { type: 'string', default: 'development' }, model: { type: 'string' } } });
  ...
  const dir = await runContinuity({ live: values.live!, provider, split: values.split as 'development' | 'evaluation', model: values.model, apiKey: process.env[provider === 'typesafe' ? 'TYPESAFE_API_KEY' : 'OPENROUTER_API_KEY'] });
