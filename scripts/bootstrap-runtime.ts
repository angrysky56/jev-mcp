...
const env={...Object.fromEntries(Object.entries(process.env).filter((e):e is [string,string]=>e[1]!==undefined)),JEV_PROVIDER:'openrouter',JEV_DATA_DIR:`${root}/.jev`};
...
try{
  ...
  await writeJson(`${root}/studies/runtime/bootstrap-${Date.now()}.json`,report);
