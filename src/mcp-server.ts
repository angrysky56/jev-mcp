...
Transport note: some hosts deliver context/state as a JSON string or reorder object keys alphabetically. Strings that parse to an object or array are unwrapped. Key order cannot be recovered. Across 10 claims, swapping two fields left clear cases unchanged (mean shift 0.01) but moved borderline ones by 0.1–0.2 and flipped one label, so compare borderline results only across runs made through the same client.

Stored catalogs: run_capability accepts records:{scope, query?, kind?, limit?} instead of items, so a reusable list is not resent on every call. Each record becomes item {title, text, kind, source, metadata?}.

Starter library (installed by npm run mcp:bootstrap when missing; see starters/capabilities.json): claim_router routes a claim {claim, basis, action?} to the check its reasoning needs; inference_warrant flags overclaiming, scope gaps and absence-as-evidence; evidence_stance sorts evidence items against context.claim and flags rival explanations for context.observations; formalization_fidelity checks whether a rewrite keeps the original's meaning; tool_router suggests which of your MCP servers fit context.task. Route descriptions name the next check; they never assume a particular server is installed.

Your tool catalog: tool_router judges only the catalog you give it. Build it from the MCP servers your host actually has: one store_records record per server, scope mcp_catalog, kind mcp_server, title = server name, text = "Does: …\nGood for: …\nNot for: …" written from that server's tool descriptions (starters/tool-catalog.example.json shows the format). Rebuild when servers change. Then run_capability {name:"tool_router", records:{scope:"mcp_catalog"}, context:{task, baseline}, summaryOnly:true}, where baseline lists what the assistant can already do without those servers (for example "a shell, file editing and web search"). With no baseline, Jev compares against an assistant that can only write text; in one 14-task run that gave 2 unneeded use routes versus 0 with a baseline.

Suggested habits for a connected agent: before a substantial task, run tool_router if a catalog exists and report whether the suggested servers helped; before reporting findings, run claim_router on the main claims and act on each route. Jev is cheap and fast, so batch independent judgments; keep counting, arithmetic, dates and safety decisions in code or a prover.
...
/** Stored records as run items; the record ID is the item ID. */
export function recordItems(runtime:CapabilityRuntime,r:{scope:string;query:string;kind?:string;limit:number}):{id:string;data:Json}[] {
  const page=runtime.store.searchRecords(r.scope,r.query,r.limit,undefined,r.kind) as {items:{id:string;title:string;text:string;kind:string;source:string;metadata?:Json}[]};
  return page.items.map(x=>({id:x.id,data:{title:x.title,text:x.text,kind:x.kind,source:x.source,...(x.metadata!==undefined?{metadata:x.metadata}:{})}}));
}
...
export function createMcpServer(runtime:CapabilityRuntime){
  ...
  register('run_capability','Apply a saved capability to a batch of source records, web results, memories, or arbitrary JSON items. Pass items directly, or records:{scope} to run over stored records (up to 20, optionally filtered by query or kind), which keeps large reusable catalogs out of the call. Each item sees shared context and its own prior-stage answers. Returns selected IDs, ranks, composite scores and routes when defined, cost, and runId; get_run retrieves all results or exact events. This makes Jev calls but executes no actions described in the data.',z.object({...referenceSchema,items:z.array(itemSchema).min(1).max(20).optional().describe('Records with stable unique IDs and JSON data. Omit when using records.'),records:z.object({scope:identifier,query:z.string().max(500).default('').describe('Optional full-text filter; empty takes all active records in scope.'),kind:recordSchema.shape.kind.optional(),limit:z.number().int().min(1).max(20).default(20)}).strict().optional().describe('Run over stored records instead of items. Each record becomes item {title, text, kind, source, metadata?}.'),context:jsonValue.default({}),summaryOnly:z.boolean().default(false).describe('Return only runId, status, costs, selectedIds, and routes, without per-item rows (read them with get_run). Use for routing.'),...runOptionsSchema}),(a,signal)=>{
    if(!!a.items===!!a.records)throw new RuntimeError('invalid_items','Give either items or records, not both.');
    const items=a.items?a.items.map(i=>({...i,data:unwrapJson(i.data as Json)})):recordItems(runtime,a.records!);
    if(!items.length)throw new RuntimeError('invalid_items',`No active records found in scope ${a.records!.scope}.`);
    const run=runtime.run(runtime.store.getCapability(a.name,a.version),items,unwrapJson(a.context as Json),{...a,signal});
    // Routing callers usually need only summary.routes and selectedIds; per-item rows stay available via get_run.
    return a.summaryOnly?run.then(({results:_,moreResults:__,...rest})=>rest):run;
  },true);
