...
Stored catalogs: run_capability accepts records:{scope, query?, kind?, limit?} instead of items, so a reusable list (for example the MCP server catalog in scope mcp_catalog used by tool_router) is not resent on every call. Each record becomes item {title, text, kind, source, metadata?}.
